...
def MakeConv(config_path, 
    ...
    final_filters = Funnel(xx.shape[-1], y_data_shape[4])
    
    for i in range(len(final_filters)):
        xx = Conv3D(filters=final_filters[i],
                        kernel_size=(3, 3, 3),
                        padding="same",
    ...
    keras.utils.plot_model(model, show_shapes=True, show_layer_activateions=True, to_file=os.path.join('SavedModels/Figs', 'Conv.png'))
    print(model.summary())
    return model
