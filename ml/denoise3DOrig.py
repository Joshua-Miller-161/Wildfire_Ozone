...
def MakeDenoise(config_path,
    ...
    x = Conv3D(filters=outer_filters, # On website 32
                kernel_size=(3, 3, 3),
                strides=(2, 2, 2),
                padding="same",
    ...
    x = Conv3D(filters=inner_filters, # On website 64
               kernel_size=(3, 3, 3),
    ...
    x = Dense(units=np.prod(pre_latent_size[1:]), activation='tanh')(x)
    x = LayerNormalization()(x)
    x = Dropout(rate=0.1)(x)
    ...
    x = Convolution3DTranspose(filters=inner_filters, # On website 64
                               kernel_size=(y_data_shape[1], 3, 3),
                               strides=(y_data_shape[1], 2, 2),
                               padding="same",
    ...
    x = Convolution3DTranspose(filters=outer_filters, # On website 32
                               kernel_size=(1, 3, 3),
                               strides=(1, 2, 2),
                               padding="same",
                               activation='linear')(x)
    ...
    autoencoder = keras.Model(input_, output, name="Denoise3DOrig")
    
    # if not (os.path.basename(__file__) == 'main.py' or __name__ == '__main__'):
