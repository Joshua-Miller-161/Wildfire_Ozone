...
def Plot_O3_Fire(ax, fire_x, fire, O3_x, O3, 
                 ...
                 num_ticks=None, fire_ylabel=None, O3_ylabel=None, draw_line_O3=None, draw_line_fire=None):
    ...
    if not(draw_line_O3==None):
        ax2.plot(O3_x, np.ones_like(O3_x)*draw_line_O3, color='deepskyblue', linewidth=1)
...
def PlotBoxes(config_path, ax, plot_text=False):
    ...
    if plot_text:
        ...
        ax.text(box1[0, 0]+1, box1[1, 0]-2, 'west ocean', fontsize=7)
        ax.text(box2[0, 0]+1, box2[1, 0]-2, 'east ocean', fontsize=7)
