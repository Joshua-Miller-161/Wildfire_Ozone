...
ax5.set_title('Smooth Fire vs. Ozone')
...
ax6.set_title('Smooth Fire vs. Fire')
for i in range(np.shape(corrs)[0]):
    for j in range(np.shape(corrs)[1]):
...
ax7.set_title('Smooth Ozone vs. Ozone')
for i in range(np.shape(corrs)[0]):
    for j in range(np.shape(corrs)[1]):
...
ax8.set_title('Smooth '+r'$\frac{dFire}{dt}$'+' vs. Ozone')
for i in range(np.shape(corrs)[0]):
    for j in range(np.shape(corrs)[1]):
...
ax9.set_title('Smooth Fire vs. '+r'$\frac{O_3}{dt}$')
for i in range(np.shape(corrs)[0]):
    for j in range(np.shape(corrs)[1]):
